...
/**
 * A component that renders LobHub icons as SVG elements.
 *
...
@Component({
  ...
  selector: 'i-lobehub, lobehub-icon',
  template: `<ng-content />`,
  styles: `
    :host {
...
export class LobehubIconComponent {
  ...
  #getSvgIcon(iconName: string | undefined) {
    ...
    const pascalIconName = upperCamelCase('icon-' + iconName);

    // Retrieve icon
    const svg = icons[pascalIconName];
    if (!svg) {
      if (!this.#options.some((option) => option.ignoreWarnings)) {
        console.warn(
          `Icon "${iconName}" not found in the configured icon sets.`
        );
      }
      return '';
    }
